...
- topic is a short subtopic label (1–3 words, e.g. "Redox", "Electrolysis", "Rate laws") grouping related items within the material. Reuse the same label for items in the same subtopic; aim for 2–8 distinct topics across the extraction.
...
{"items":[{"statement":string,"kind":"fact"|"concept"|"distinction"|"procedure","topic":string,"distractors":[string,string,string]}],"edges":[{"a":number,"b":number,"relation":"contrasts_with"|"depends_on"|"related_to","weight":number}],"richer":boolean}
